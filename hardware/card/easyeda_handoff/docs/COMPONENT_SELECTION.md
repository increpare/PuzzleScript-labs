...
For the live footprint-import checklist, see
[`FOOTPRINT_LOCK_MATRIX.md`](FOOTPRINT_LOCK_MATRIX.md). That file separates
package-locked parts from sample and EasyEDA/JLC import gates.
...
| DSI connector | 15-pin 1.0 mm right-angle FFC, Hirose FH12-15S-1SH(55) class | SMT FFC/FPC connector | Pinout captured, physical orientation gated | Must match the Waveshare/Raspberry-Pi-style DSI cable orientation before routing. Waveshare defines pins 14 and 15 as 3V3. See `DSI_PANEL_INTERFACE.md` for the same-side/opposite-side cable and card-end photo checklist. |
| USB-C | HRO TYPE-C-31-M-12 USB-C 2.0 receptacle | `Connector_USB:USB_C_Receptacle_HRO_TYPE-C-31-M-12`, mounted on **PCB back**, top edge | Footprint locked 2026-07-09 | Replaces the mid-mount: no board notch, no display-gap constraint. Rear shell gets a local ~1 mm bump at the top edge (`depthProfile.usbBump`). Re-check JLC/EasyEDA stock/library match before order. |
...
| Charger / power path | TI BQ24075RGTR | 3 x 3 mm VQFN-16 | Package locked | Standalone 1S linear charger with power path and SYSOFF. Set ISET conservatively around 0.5 C unless thermal testing proves the sealed card can dissipate more. |
| Fuel gauge | MAX17048G+T10 | 2 x 2 mm LFCSP/TDFN-8, not WLP | Package locked | Simple 1S I2C gauge, no sense resistor. Use the G+T10 package, not the X/WLP variant. |
| 3V3 regulator | TI TPS63070RNM family | 3 x 2.5 mm QFN buck-boost | Package locked | Replaces the old TPS62135 buck placeholder. |
| Panel load switch | TPS22919DCK | 2 x 2.1 mm SOT-SC70-6 | Package locked | U6 gates `+3V3_PANEL` so sleep current is not dominated by the display module. |
...
| Haptic driver | TI DRV2605LDGS | VSSOP-10 preferred for hand assembly/debug, DSBGA only if needed | Package locked | I2C LRA/ERM driver with library and auto-resonance support. |
...
- `J3`: place at `CONN_DSI_FFC`; route DSI before anything else after
  `DSI_PANEL_INTERFACE.md` closes the same-side/opposite-side cable and
  card-end contact photo check.
...
Stable gate IDs and close criteria live in
[`FOOTPRINT_LOCK_MATRIX.md`](FOOTPRINT_LOCK_MATRIX.md#open-gate-ids). The list
below is the human summary:
...
- DSI FFC same-side/opposite-side cable parity, card-end contact photo, latch
  side, cable exit direction, and pin-1 orientation with the actual Waveshare
  panel.
...
- JLC-first footprint matrix: `hardware/card/FOOTPRINT_LOCK_MATRIX.md`.
- DSI pinout and physical closeout checklist: `hardware/card/DSI_PANEL_INTERFACE.md`.
...
- Waveshare 43H-800480 product page: https://www.waveshare.com/43h-800480-ips.htm
- Waveshare 43H-800480 wiki: https://www.waveshare.com/wiki/43H-800480-IPS
- Waveshare 43H-800480 STEP drawing: https://files.waveshare.com/upload/c/cd/43h-800480-IPS.zip
