...
> directory's JSON/generator and the `case/out/pcb` project are temporary legacy
> compatibility artifacts. Never use them to overwrite `electronics/`.
...
- `connectivity.json` is the legacy compatibility electrical contract:
  ...
  Make legacy-model electrical changes here.
...
regeneration replaces it. The drawing uses short wires with local labels
