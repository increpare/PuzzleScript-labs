...
std::vector<ps_input> loadReplay(const std::filesystem::path& path) {
    ...
    while (std::getline(input, line)) {
        ...
        if (!parseInputToken(line, &parsed)) {
            fprintf(stderr, "gbc_any_mask_oracle_smoke: unknown replay token: %s\n", line.c_str());
            return {};
        }
        ...
        for (int32_t y = 0; y < status.height; ++y) {
            ...
            for (int32_t object = 0; object < ps_gbc_generated_game.object_count; ++object) {
                ...
                if (nativeHas != gbcHas) {
                    fprintf(
                        stderr,
                        "gbc_any_mask_oracle_smoke: %s differs at %d,%d object=%d\n",
                        phase,
                        x,
                        y,
                        object);
...
int main() {
    const std::filesystem::path replayPath =
        ...
        / "tests"
        / "fixtures"
        / "gbc_any_object_mask_replay.txt";
    const std::vector<ps_input> inputs = loadReplay(replayPath);
    if (inputs.empty()) {
        fprintf(stderr, "gbc_any_mask_oracle_smoke: replay fixture is empty or invalid\n");
        return 1;
    }
    ...
    if (!ps_compile_source(source.data(), source.size(), &compilation) || compilation == nullptr) {
        fprintf(stderr, "gbc_any_mask_oracle_smoke: native compilation failed\n");
        return 1;
    }
    ...
    if (nativeGame == nullptr) {
        fprintf(stderr, "gbc_any_mask_oracle_smoke: compiled game transfer failed\n");
        return 1;
    }
    if (!ps_full_state_create(nativeGame, &native, &error)) {
        fprintf(
            stderr,
            "gbc_any_mask_oracle_smoke: native session failed: %s\n",
    ...
    if (!ps_full_state_load_level(native, 0, &error)) {
        fprintf(
            stderr,
            "gbc_any_mask_oracle_smoke: native level load failed: %s\n",
            error == nullptr ? "unknown" : ps_error_message(error));
        ps_free_error(error);
    ...
    if (!initGbcSession(&gbc)) {
        fprintf(stderr, "gbc_any_mask_oracle_smoke: gbc session init failed\n");
        ps_full_state_destroy(native);
        ps_free_game(nativeGame);
        return 1;
    }
    ...
    for (size_t index = 0; index < inputs.size(); ++index) {
        ...
        if (!resultsEqual(nativeResult, gbcResult)) {
            fprintf(
                stderr,
                "gbc_any_mask_oracle_smoke: native vs gbc result flags differ at step %zu\n",
                index);
            ps_full_state_destroy(native);
            ps_free_game(nativeGame);
            return 1;
    ...
    ps_full_state_destroy(native);
    ps_free_game(nativeGame);
    printf("gbc_any_mask_oracle_smoke: ok\n");
    return 0;
}
