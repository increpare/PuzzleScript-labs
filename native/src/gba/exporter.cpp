...
std::string emitGeneratedHeader(size_t sessionBytes) {
    ...
    out << "#pragma once\n\n#include \"puzzlescript/gba.h\"\n\n"
        ...
        "ps_gba_kernel_result ps_gba_generated_turn(uint32_t*, uint32_t, uint16_t, uint16_t, "
...
std::string emitRules(const Game& game, const std::filesystem::path& sourcePath, uint64_t sourceHash) {
    ...
    compiler::emitCompactTurnBackend(kernel, game, sourcePath.string(), sourceHash, 0, compiler::CompactCodegenOptions{});
    ...
    out << "\n} // namespace\n\n"
        ...
        << "    uint32_t* boardWords, uint32_t boardWordCount, uint16_t width, uint16_t height,\n"
        ...
        << "    if (boardWords == nullptr || rng == nullptr) return ps_gba_kernel_result{};\n"
        << "    ps_gba_pending_message = nullptr; ps_gba_pending_sound_count = 0;\n"
        << "    if (resetScratch) scratch = puzzlescript::Scratch{};\n"
        << "    levelState.board.objects.resize(boardWordCount);\n"
        << "    std::memcpy(levelState.board.objects.data(), boardWords, boardWordCount * sizeof(uint32_t));\n"
        << "    std::memcpy(levelState.rng.s.data(), rng->s, sizeof(rng->s));\n"
        ...
        << "    if (levelState.board.objects.size() == boardWordCount)\n"
        << "        std::memcpy(boardWords, levelState.board.objects.data(), boardWordCount * sizeof(uint32_t));\n"
...
ExportResult exportGame(const ExportOptions& options) {
    ...
    const size_t fixedSessionBytes = 2048U + boardStateBytes * 2U;
    ...
    writeFileIfChanged(result.generatedSourcePath, emitGeneratedSource(
        ...
        maxCells, undoCapacity));
    writeFileIfChanged(result.generatedRulesPath, emitRules(game, options.sourcePath, sourceHash));
