...
static bool ps_gbc_facade_rules_patterns_match_at(
    const ps_gbc_session* session,
    const ps_gbc_runtime_pattern* pattern,
...
static const ps_gbc_runtime_pattern* ps_gbc_facade_rules_pattern_table(
    const ps_gbc_session* session,
...
static bool ps_gbc_facade_rules_rule_matches_at(
    ...
    const ps_gbc_runtime_pattern* pattern =
        ps_gbc_facade_rules_pattern_table(session, rule);
    if (pattern == NULL) return false;
    return ps_gbc_facade_rules_patterns_match_at(
        session, pattern, rule->pattern_count, start, delta);
}

static uint8_t ps_gbc_facade_rules_collect_pattern_matches(
    ps_gbc_session* session,
    ...
    if (use_player_anchor) {
        ...
        for (player_index = 0U;
            ...
            if (ps_gbc_facade_rules_patterns_match_at(
...
static uint8_t ps_gbc_facade_rules_collect_matches(
    ...
    return ps_gbc_facade_rules_collect_pattern_matches(
        session,
        patterns,
        rule->pattern_count,
        rule->direction,
...
static bool ps_gbc_facade_rules_apply_pattern_replacements(
    ps_gbc_session* session,
    const ps_gbc_runtime_pattern* pattern,
    uint8_t pattern_count,
    uint8_t start,
...
static bool ps_gbc_facade_rules_apply_rule(
    ...
    int8_t delta;
    bool changed = false;
    patterns = ps_gbc_facade_rules_pattern_table(session, rule);
...
#endif
    delta = ps_gbc_facade_rules_delta(session, rule->direction);
    if (rule->row_count >= 2U) {
        ...
        row0_count = ps_gbc_facade_rules_collect_pattern_matches(
            session,
            patterns,
            row0_len,
            rule->direction,
        ...
        row1_count = ps_gbc_facade_rules_collect_pattern_matches(
            session,
            row1_patterns,
            row1_len,
            rule->direction,
            ...
            for (row1_index = 0U; row1_index < row1_count; ++row1_index) {
                ...
                if (!first_tuple) {
                    if (!ps_gbc_facade_rules_patterns_match_at(
                        ...
                        || !ps_gbc_facade_rules_patterns_match_at(
                            session, row1_patterns, row1_len, start1, delta)) {
                        still_matches = false;
                    }
                }
                if (still_matches) {
                    changed = ps_gbc_facade_rules_apply_pattern_replacements(
    ...
    {
        const uint8_t match_count = ps_gbc_facade_rules_collect_matches(
            session, rule, delta);
        ...
        for (match_index = 0U; match_index < match_count; ++match_index) {
            ...
            if (!ps_gbc_facade_rules_rule_matches_at(
                    session, rule, start, delta)) continue;
            changed = ps_gbc_facade_rules_apply_pattern_replacements(
