...
static void writeNamespaceHeader(
    ...
    if (prefix.empty()) {
        ...
        for (unsigned pack = 0U; pack < 256U; ++pack) {
            out << "#define ps_gbc_specialized_rule_pack_" << pack
                << " " << prefix << "_ps_gbc_specialized_rule_pack_"
                << pack << "\n";
        }
        out << "\n";
    }
...
std::string emitBankedSourceWrapper(
    unsigned bank,
    const char* source,
    bool facade
) {
    ...
    if (facade) {
        out << "#define PS_GBC_GENERATED_FACADE_WRAPPER 1\n";
    }
    out << "#include \"" << source << "\"\n";
    return out.str();
}
...
std::string emitHeader(
    ...
    out << "#ifndef PS_GBC_GENERATED_GAME_H\n#define PS_GBC_GENERATED_GAME_H\n\n"
        ...
        << "#define PS_GBC_GENERATED_ROM_BANK " << gameCoreBank << "U\n\n"
...
SpecializedTurnExportInfo writeSpecializedTurnArtifacts(
    ...
    // near-calls ps_gbc_facade_apply_groups from a different generated bank. An
    // MBC5 cart has one switchable window, so both the reads and the call land
    // in whichever bank the caller occupies. There is nothing to specialize
    // for such a game anyway, so decline here and let core.c's interpreted turn
    // run it.
    ...
    if (info.supported) {
        ...
        for (const compiler::GbcSpecializedTurnSourceFile& sourceFile : emitResult.files) {
            ...
            info.highestBank = std::max(
                info.highestBank, sourceFile.bankNumber);
        }
        writeFileIfChanged(
            outputDirectory / "specialized_sources.list",
            sourcesList.str());
...
ExportResult exportGame(const ExportOptions& options) {
    ...
    compiler::GbcSpecializedTurnEmitOptions turnEmitOptions;
    turnEmitOptions.mainBank = facadeBank + 1U;
    turnEmitOptions.firstRulesBank = turnEmitOptions.mainBank + 1U;
    ...
    const auto requireStagedTextFits = [](const std::string& text) {
        if (text.size() >= 256U) {
            throw std::runtime_error(
                "GBC text exceeds the 255-byte staging limit");
        }
    };
    requireStagedTextFits(
        metadataValue(game, "title", "PuzzleScript Game"));
    requireStagedTextFits(metadataValue(game, "author", ""));
    for (const PackedLevel& level : levels) {
        requireStagedTextFits(level.messageText);
    }
    for (const PackedRule& rule : rules) {
        requireStagedTextFits(rule.message);
    }
    ...
    writeFileIfChanged(result.generatedSourcePath, emitSource(
        ...
        maxCells, undoCapacity, objectCellBytes, gameCoreBank));
    writeFileIfChanged(
        options.outputDirectory / "generated_core.c",
        emitBankedSourceWrapper(gameCoreBank, "core.c", false));
    writeFileIfChanged(
        options.outputDirectory / "generated_compact_facade.c",
        emitBankedSourceWrapper(facadeBank, "compact_facade.c", true));
    writeFileIfChanged(
        options.outputDirectory / "generated_facade_rules.c",
        emitBankedSourceWrapper(facadeBank, "facade_rules.c", true));
    ...
    const unsigned highestBank = std::max(
        facadeBank, specializedTurnExport.highestBank);
    if (highestBank > 255U) {
        throw std::runtime_error("GBC per-game bank range exceeds bank 255");
    }
    const unsigned nextBank = highestBank + 1U;
    ...
    manifest << "{\n"
        ...
        << "  \"game_core_bank\": " << gameCoreBank << ",\n"
        << "  \"facade_bank\": " << facadeBank << ",\n"
        << "  \"specialized_main_bank\": " << turnEmitOptions.mainBank << ",\n"
        << "  \"specialized_first_rules_bank\": "
        << turnEmitOptions.firstRulesBank << ",\n"
        << "  \"next_bank\": " << nextBank << ",\n"
        << "  \"cgb_only\": true,\n"
