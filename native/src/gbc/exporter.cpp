...
std::string emitHeader(size_t sessionBytes, uint8_t movementBytesPerCell) {
    std::ostringstream out;
...
std::string emitSource(
    ...
    for (size_t index = 0; index < levels.size(); ++index) {
        if (!levels[index].message) {
            emitUnsignedArray(out, "uint32_t", ("kLevel" + std::to_string(index) + "Cells").c_str(),
                levels[index].cells, "U");
        }
    }
...
ExportResult exportGame(const ExportOptions& options) {
    ...
    const auto requiredBytesForUndo = [&](uint8_t undo) {
        (void)undo;
        return static_cast<size_t>(PS_GBC_SESSION_OVERHEAD_BUDGET)
            + static_cast<size_t>(maxCells) * sizeof(uint32_t)
            + static_cast<size_t>(maxCells) * movementLayout.bytesPerCell
    ...
    for (const PackedLevel& level : levels) {
        estimatedGameBankBytes += level.cells.size() * sizeof(uint32_t)
    ...
    writeFileIfChanged(result.generatedHeaderPath,
        emitHeader(sessionBytes, movementLayout.bytesPerCell));
    writeFileIfChanged(result.generatedSourcePath, emitSource(
        ...
        static_cast<uint8_t>(viewportHeight), maxCells, undoCapacity));
    ...
    manifest << "{\n"
        ...
        << "  \"snapshot_sram_bytes\": "
        << static_cast<size_t>(maxCells) * sizeof(uint32_t) * (undoCapacity + 1U) << ",\n"
