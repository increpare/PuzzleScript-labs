...
function processInput(dir, dontDoWin, dontModify, skipAgainProbe) {
	...
	let inputindex = dir;
	const skipMaskRebuildForAgain = inputindex < 0;
	let playerPositions = [];
	...
	const masksFreshFromRestore = skipMaskRebuildForAgain && level.solverMasksFreshFromRestore;
	if (masksFreshFromRestore) {
		level.solverMasksFreshFromRestore = false;
	} else {
