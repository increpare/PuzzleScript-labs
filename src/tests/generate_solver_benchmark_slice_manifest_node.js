...
fs.writeFileSync(path.join(messageCorpusDir, 'comment-message-first.txt'), 'title comment message first\nobjects\nplayer\nrules\n[ > player ] -> [ > player ]\nlevels\n(multiline note\nmiddle (aside) still note\nstill note)\nmessage intro\np.\n');
...
assert.strictEqual(message.targets.find((target) => target.game === 'message-first.txt').level, 1);
assert.strictEqual(message.targets.find((target) => target.game === 'comment-message-first.txt').level, 1);
