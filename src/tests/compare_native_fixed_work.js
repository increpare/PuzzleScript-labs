...
const report = {
    scope: 'Serial alternating native BFS candidate-level solves after one warmup per binary. Same expansion cap; a wall deadline before the cap is an error. Process wall includes source compilation and replay validation.',
    max_expanded: cap,
...
function run(which) {
    ...
    const child = spawnSync(binaries[which], [corpus, String(cap)], { encoding: 'utf8', windowsHide: true, timeout: 300000, maxBuffer: 32000000 });
