...
for (let index = 3; index < args.length; index++) {
    ...
    if (arg === '--runs' && index + 1 < args.length) {
        ...
        const jobsArg = args[++index];
        if (jobsArg === 'auto') {
            jobs = Math.max(1, os.cpus().length);
        } else {
            jobs = parsePositiveInt(jobsArg, '--jobs');
        }
...
function runTarget(target, runIndex, strategy, timeoutMs) {
    ...
    return new Promise((resolve, reject) => {
        const child = spawn(solverPath, solverArgs, {
        ...
        child.on('close', (status) => {
            ...
            if (status !== 0) {
                reject(new Error(`solver exited ${status} for ${targetKey(target)} run=${runIndex + 1}\nstdout:\n${stdout}\nstderr:\n${stderr}`));
                return;
            }
            let json;
            try {
                ...
                reject(new Error(`solver returned invalid JSON for ${targetKey(target)} run=${runIndex + 1}: ${error.message}`));
                return;
            }
            if (json.totals.errors !== 0) {
                reject(new Error(`solver reported errors=${json.totals.errors} for ${targetKey(target)} run=${runIndex + 1}`));
                return;
            }
            if (json.results.length !== 1) {
                reject(new Error(`expected one result for ${targetKey(target)}, got ${json.results.length}`));
                return;
            }
            const solverResult = json.results[0];
            if (solverResult.game !== target.game || solverResult.level !== target.level) {
                reject(new Error(`solver returned ${resultKey(solverResult)} for requested ${targetKey(target)}`));
                return;
            }
            const sample = {
                run: runIndex + 1,
                ...
                runtime_counters: profileRuntimeCounters ? parseRuntimeCounters(stderr) : null,
            };
...
async function summarizeTarget(target, strategy) {
    ...
    for (let index = 0; index < runs; index++) {
        const sample = await runTarget(target, index, strategy, timeoutMs);
        samples.push(sample);
...
async function main() {
    ...
    const summaries = await runLimited(targets, jobs, async (target) => summarizeTarget(target, strategy));
