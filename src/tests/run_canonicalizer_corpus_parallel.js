...
// Parallel launcher for run_canonicalizer_corpus_audit.js.
//
// Usage:
//   node src/tests/run_canonicalizer_corpus_parallel.js \
//     --corpus PATH --jobs 8 --log-dir build/canonicalizer-audit-giant
...
function usage() {
    return [
        'Usage: node src/tests/run_canonicalizer_corpus_parallel.js --corpus PATH --jobs N [options]',
        '',
...
function runShard(workerScript, options, range) {
    ...
    return new Promise(resolve => {
        ...
        process.stderr.write(`run_canonicalizer_corpus_parallel: starting shard ${range.shard} [${range.start},${range.end}) -> ${shardLogDir}\n`);
...
function writeProgressSnapshot(logDir, manifest, ranges, startedAtMs) {
    ...
    process.stderr.write(`run_canonicalizer_corpus_parallel: ${formatAuditProgressLine(progress, elapsedMs)}\n`);
    return snapshot;
}
...
async function main() {
    ...
    if (!options.logDir) {
        options.logDir = path.join(process.cwd(), 'build', 'canonicalizer-audit', timestampSlug());
    }
    ...
    const workerScript = path.join(__dirname, 'run_canonicalizer_corpus_audit.js');

    await resolveParallelPartialRunAction(options, options.logDir, ranges);

    if (options.fresh) {
        clearShardCheckpoints(options.logDir, ranges);
        process.stderr.write('run_canonicalizer_corpus_parallel: restarting fresh (shard checkpoints cleared)\n');
    } else if (options.resume) {
        process.stderr.write(`run_canonicalizer_corpus_parallel: continuing partial run (${countShardCheckpoints(options.logDir, ranges)} shard checkpoints)\n`);
    }
    ...
    process.stderr.write(`run_canonicalizer_corpus_parallel: corpus=${options.corpusPath} games=${games.length} jobs=${options.jobs} shards=${ranges.length} log-dir=${options.logDir}\n`);
    process.stderr.write(`run_canonicalizer_corpus_parallel: live progress -> ${path.join(options.logDir, 'progress.json')} (every ${PROGRESS_POLL_MS / 1000}s)\n`);
    ...
    process.stderr.write(`run_canonicalizer_corpus_parallel: finished shards=${ranges.length} failures=${aggregate.failures} failed_shards=${aggregate.failedShards.length}\n`);
    process.stderr.write(`run_canonicalizer_corpus_parallel: aggregate=${path.join(options.logDir, 'aggregate-summary.json')}\n`);

    if (aggregate.failedShards.length > 0) {
        process.stderr.write(`run_canonicalizer_corpus_parallel: failed (${aggregate.failedShards.length} shard(s): ${aggregate.failedShards.join(', ')})\n`);
        process.exit(1);
    }
    if (aggregate.failures > 0 && options.exitOnFailure) {
        process.exit(1);
    }
    console.log(`run_canonicalizer_corpus_parallel: ok (log-dir=${options.logDir})`);
}

main().catch(error => {
    process.stderr.write(`run_canonicalizer_corpus_parallel: fatal ${error && error.stack ? error.stack : error}\n`);
    process.exit(1);
});
