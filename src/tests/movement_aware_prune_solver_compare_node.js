...
function usage() {
    process.stderr.write(
        'Usage: node src/tests/movement_aware_prune_solver_compare_node.js <corpus_dir> [runner args...]\n',
    );
    process.exit(2);
}
...
function main(argv = process.argv) {
    ...
    if (failures.length > 0) {
        process.stderr.write('movement_aware_prune_solver_compare_node: failed\n');
    ...
    process.stdout.write(
        `movement_aware_prune_solver_compare_node: ok levels=${offTotals.levels || 0} ` +
