...
if (process.argv.length < 5) throw new Error('Usage: compare_native_runtime.js BASELINE_DIR CANDIDATE_DIR OUTPUT_JSON [PAIRS=5] [replay|generator|all]');
...
if (suite !== 'replay') for (const game of ['cakemonsters', 'chaos wizard', 'dropswap', 'midas']) specs.push({ name: game, exe: 'puzzlescript_generator' + executableSuffix });
for (const spec of specs) {
    ...
    for (let p = 0; p < pairs; ++p) {
        ...
        if (spec.name === 'replay') {
            ...
            for (const counter of ['rules_visited', 'replacements_applied']) assert.strictEqual(pair.baseline.runtime_counters[counter], pair.candidate.runtime_counters[counter], counter);
        }
        row.pairs.push(pair);
