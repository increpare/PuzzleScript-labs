...
## Interaction with HDA multicore (measured)

The whole point of this work is the multicore solver (`--hda-jobs N`), so the key
question is how the per-step speedup composes with HDA parallelism. Measured on
the two *correct* probe games under `--hda-jobs 8`, both at steady state
(timeout level, expanded-nodes/s) and on the short focus solves.

**Short focus solves at `--hda-jobs 8` (time-to-solve, median of 5):**
heroes_of_sokoban_3#10 75→43 ms (**1.74×**), cakemonsters#49 80→55 ms (**1.45×**).
Lower than the single-thread step win — at 40–80 ms, HDA startup/race overhead
dominates and masks it. The longer the search, the closer to the steady-state
ratio below.

**Steady-state throughput sweep (expanded in 1 s; interp = full storage, compact = `--compact-node-storage`):**

| game / HDA jobs | interp | compact | compact/interp | interp scaling 1→8 | compact scaling 1→8 |
|---|---|---|---|---|---|
| heroes_of_sokoban_3 #21 — 1 | 21,022 | 29,438 | 1.40× | | |
| heroes_of_sokoban_3 #21 — 8 | 89,547 | 116,596 | **1.30×** | 4.26× (53%) | 3.96× (**50%**) |
| cakemonsters #34 — 1 | 29,505 | 81,754 | 2.77× | | |
| cakemonsters #34 — 8 | 115,559 | 265,521 | **2.30×** | 3.92× (49%) | 3.25× (**41%**) |

**Findings:**

1. **Composes, but sub-multiplicatively.** The compact/interp ratio *shrinks* with
   thread count (heroes_3 1.40→1.30×, cakemonsters 2.77→2.30×). The single-thread
   2–2.75× lands as ~1.3–2.3× at HDA-8 — a real, large multicore win, but not the
   full multiply.
2. **It slightly *lowers* parallel efficiency** (50% vs 53%; 41% vs 49%). Speeding
   up `step` shrinks the dominant compute, so **HDA coordination becomes the new
   ceiling**: each thread generates children ~2.75× faster, so the single global
   `outstandingWork` atomic, the per-message inbox mutex, cross-shard messaging,
   and visited inserts all fire ~2.75× more often. That fixed sync cost — hidden
   today behind the slow step — then eats the speedup (cakemonsters 49%→41% is the
   fingerprint). The optimistic "less allocation → better bandwidth → higher
   efficiency" hypothesis did **not** materialize; the Amdahl/coordination shift
   dominates.
3. **The two games bracket the effect by how step-heavy they are.** cakemonsters
   (step-dominated, 2.75× single-thread) shows the bigger win *and* the bigger
   efficiency loss; heroes_3 (more time in HDA/visited bookkeeping, high duplicate
   rate, 2× step) shows a smaller win barely above coordination noise.

**Consequence for sequencing:** compact-turn is the biggest single multicore lever
(~1.3–2.3× at HDA-8), but it is *self-limiting* — the faster `step` gets, the more
the HDA coordination layer caps the result. So compact-turn should land first,
then the now step-light HDA loop should be re-profiled and its coordination
attacked (see milestone 6). PR #3's per-shard buffer pool already removed one
cross-thread hazard ahead of that and stacks cleanly.

> Reproduce: run `/tmp/solver_pool` (generic) vs the specialized binary under
> `build/compiled-rules-builds-Ninja/solver-focus-*/native/puzzlescript_solver`
> with `--hda-jobs 1/2/4/8 --strategy hda-weighted-astar --no-solutions --json`
> (`--compact-node-storage` on the specialized one) on a timeout level, and
> compare `expanded`.
...
6. **(Follow-on) attack the HDA coordination ceiling that compact-turn exposes.**
   Once `step` is ~2× cheaper, parallel efficiency is limited by the sync layer
   (measured 41–50% at HDA-8, *worse* than the interpreter — see "Interaction with
   HDA multicore"). Re-profile the step-light loop and target: the single global
   `outstandingWork` atomic (shard it / use relaxed accounting), the per-message
   `inboxMutex` (lock-free or batched hand-off), and spin/`yield()` termination
   (condition-variable wait). Near-worthless today (~2–5% behind the slow step);
   the main lever *after* compact-turn lands. Validate with the `--hda-jobs`
   scaling sweep above.
