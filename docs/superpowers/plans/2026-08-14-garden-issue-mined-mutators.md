...
- The test at `src/tests/monster_garden/tests.js:136` asserts the exact list of mutator names in order. **Every task that adds a mutator must append its name to that list**, or the suite fails. The registry now holds 53 mutators; locate the array's end by content, not by the line numbers in this plan.
- These mutators produce *invalid or damaging* programs on purpose. They must never declare an `equivalence` field.
- **Every mutator test must compile its mutant.** This is the hard-won lesson from the equivalence-oracle plan, where an entire class of defects shipped green because tests asserted only on the mutated *string*. Use the existing `workerResult()` helper in `tests.js`. For these damaging mutators the assertion is not that the mutant is `ok` — most will not be — but that the worker returns a **known result kind** (`garden.KNOWN_RESULT_KINDS`) rather than a `crash` originating in the garden's own code. A mutator that reliably crashes the harness rather than the compiler is a defect in the mutator.
- **Seed loops need an applied-at-least-once guard.** `if (!result) continue;` with no `assert(succeeded > 0)` is a test that can pass while exercising nothing.
- **Identifier regexes must be case-insensitive.** PuzzleScript identifiers are case-insensitive, and this hazard was found three separate times during the equivalence-oracle work. Any `new RegExp('\\b' + name + '\\b')` built from an object or legend name needs the `i` flag.
...
```js
// 'v' is PuzzleScript's "down" movement glyph. Unlike '>', '<' and '^' it is
// identifier-shaped, so it passes the token test below and must be excluded
// explicitly — real corpus rules contain cells like `[ v Player | ... ]`.
...
### Task 10: `comment-in-rule`

Issue #1128 asked for a better error when a parenthetical appears inside a rule. The fix was
a deliberate rejection with the message "You can't have comments inside rules, sorry"
(`compiler.js`). The equivalence-oracle plan originally tried to make this a
*semantics-preserving* mutator on the false premise that such comments were legal; that was
wrong and was abandoned. The construct is still worth fuzzing — as a **malformity** mutator
asserting the compiler errors cleanly rather than crashing, which is what the rest of Group A
does.
...
**Interfaces:**
- Consumes: `mutateArrowRule`.
- Produces: registry entry `{ name: 'comment-in-rule', apply: commentInRule }`.

- [ ] **Step 1: Write the failing test.**

```js
test('comment-in-rule puts a parenthetical inside the brackets and the compiler rejects it cleanly', async function() {
    ...
    for (let seed = 0; seed < 20; seed++) {
        ...
        succeeded++;
        assert(/\[\s*\(garden\)/.test(result.source), 'seed ' + seed + ': ' + result.source);
        assert.strictEqual(result.source.split('\n').length, SAMPLE.split('\n').length);
    }
    assert(succeeded > 0, 'expected comment-in-rule to apply for at least one seed');

    // The point of the mutator: the compiler must reject this, not crash on it.
    const mutated = mutator.apply(SAMPLE, new garden.Random(0), {});
    const compiled = await workerResult({ source: mutated.source, inputs: [], level: 0, randomSeed: null });
    assert(
        garden.KNOWN_RESULT_KINDS.indexOf(compiled.kind) >= 0,
        'worker returned an unknown kind: ' + compiled.kind
    );
    assert.notStrictEqual(compiled.kind, 'crash', 'a comment inside a rule must not crash the compiler');
});
```
...
Run: `node src/tests/monster_garden/tests.js`
Expected: `comment-in-rule should be registered`.

- [ ] **Step 3: Implement.** Add to `src/tests/monster_garden/garden.js` before `const mutators = [`:

```js
// Issue #1128: a parenthetical inside a rule. compiler.js rejects this deliberately
// ("You can't have comments inside rules, sorry"); this mutator checks the rejection
// stays a clean error rather than becoming a crash.
...
- [ ] **Step 4: Register it** after the `multi-fault` entry:
...
- [ ] **Step 5: Update the name list** in `tests.js` — append `'comment-in-rule'`.
...
```bash
git add src/tests/monster_garden/garden.js src/tests/monster_garden/tests.js
git commit -m "Add the comment-in-rule garden mutator (issue #1128)."
```

---

### Task 11: Campaign smoke check and documentation
...
node src/tests/monster_garden/run.js \
  ...
  --mutator no-x-with-x,relative-direction-cell,same-layer-cell,property-in-concrete-slot,rigid-prefix,sprite-matrix-resize,restart-again-message,multi-fault,comment-in-rule \
