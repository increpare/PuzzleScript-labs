...
**Next:** Sokoban vertical slice for real unrolled GbdC rule emission —
see [`2026-07-24-gbc-unrolled-gbdC-sokoban-slice-design.md`](./2026-07-24-gbc-unrolled-gbdC-sokoban-slice-design.md).
The shipping specialized turn still uses the façade rule walker for early/late
groups plus shared movement resolve — correct and banked, but not yet the
desktop-style unrolled matchers the speed target assumes. mGBA solution-replay
timing on cart remains the gating measurement once unrolled emission lands.
