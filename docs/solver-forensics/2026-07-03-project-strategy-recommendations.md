...
| S12 / N8 win-relevance pruning | repaired opt-in; strong smoke candidate | Native smoke initially exposed an unsound movement-resolution gap (a solved game became exhausted); all movement writers are now conservative relevance roots and the real-game regression canary passes. Repaired three-run native smoke keeps the 99/48/3 sample split, removes 235 rules across 17/50 games, and improves aggregate generated-state throughput 88.39 -> 112.17 states/ms (+26.9%). The four-word one-run pair keeps 1/53 and improves 2.991 -> 3.485 states/ms (+16.5%). JS smoke parity/replay passes. Keep explicit until full-corpus native parity and practical hint delivery are complete |
