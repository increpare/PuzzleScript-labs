...
| P2 / N3 movement-aware prune | **rejected as default** | naive runtime masks failed on Karamell; S1-certified JS consumer path exists behind `--solver-certified-wake-prune`, but smoke-50 `step_ms` regressed 8355.9/8343.7 -> 9224.6/9203.7 vs default and 9151.5/9150.9 -> 9184.8/9192.5 vs incremental-only. The 2026-07-07 `--solver-wake-prune-counters` pass explains why: smoke-50 certified pruning skipped only 124,794 / 7,867,745 rule checks (1.59%), while 98.41% still reached `tryApply`; keep explicit-only until a narrower counter-proven site wins |
...
| N4-N7, N9 | next runtime tier by residual per-step cost; N4a done, N4b/N5a/N7 rejected; N4c promoted only for wide object masks | N4a's add-only movement dirty guard is a small validated win (+2.0% smoke-50 and +1.7% named portfolio raw step-throughput vs N1, solved splits unchanged); N4b rebuild-on-read reduced the tiny `push_goal` dirty counter canary but regressed focus-50 median wall +20.3% in the stale-board version and +51.5% with exact board reads, so do not revive broad phase-boundary rebuild deferral; N4c exact object refcount masks reuse the existing object-cell index, eliminating 96.4M secondary board-seed scans. Forcing N4c across smoke-50 was only noise-level (+0.46% throughput vs the A/N4c/A baseline midpoint), but the four-word portfolio gained +3.36% throughput and a nine-run three-word target check improved median step time 1.037 -> 0.963ms (-7.2%); N4c now enables automatically at `Game::wordCount >= 3`, with env `=0`/`=1` baseline/force controls. N7 preflight found 47.0% zero object-mask words on the four-word portfolio, but the compile-time per-cell word-plane prototype still regressed aggregate generated-state throughput 3.152 -> 2.935 states/ms (-6.9%) and materialization +15.5%, while smoke-50 was noise-level; the prototype was backed out. Do not revive per-cell SoA or runtime-selectable detailed counters. A future N7 needs plane-wise batched matching; next low-level runtime work should prefer N9/S2 or a narrowly counter-proven consumer |
| S12 / N8 win-relevance pruning | repaired opt-in; parity-clean audit, not default-ready | Native measurement exposed two implicit engine dependencies missing from ordinary rule def-use: movement writers, then object writers on mover collision layers (door rules in `pupush`). Both are now conservative roots with real-game canaries. Corrected three-run smoke keeps 99/48/3, removes 234 rules across 17/50 games, and improves aggregate generated-state throughput 88.39 -> 111.92 states/ms (+26.6%). The corrected full-corpus 250ms/eight-job run keeps errors at zero and exhausted levels 7/7, moves solved 743 -> 747, removes 684 rules, and improves throughput +6.2%; all 747 optimized solutions replay. Eight of nine fixed-budget losses recover single-job at 5s; one valid solution remains a severe tie-break regression. Keep explicit pending practical hint delivery, paired full-corpus repeats, and a fixed-budget regression policy |
