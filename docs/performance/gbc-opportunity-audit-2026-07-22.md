...
single-pass groups. After that, use object-position bitsets or posting lists to
...
The analyzer also marks these real source groups `single_pass_safe`: Sokoban
1/1, large board 0/1, rule-heavy 1/27, object-heavy 1/7, and two-lane 7/7.
The GBC runtime currently always performs a no-change confirmation pass after a
changing group. Consuming the certificate should be particularly valuable for
the two-lane case. Measure it independently because a safe group that does not
change already stops after its first pass.
